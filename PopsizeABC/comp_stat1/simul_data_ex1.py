...
outfile_name = sys.argv[1]       #prefix
nb_rep = int(sys.argv[2]) # number of simulated datasets
nb_seg = int(sys.argv[3]) # number of independent segments in each dataset
n = int(sys.argv[4]) # haploid sample size
import math
mac=int(math.ceil(n*0.1)) 
mac_ld=int(math.ceil(n*0.2)) 
...
r_max=0.5*10**(-8) # upper bound for the per generation per bp recombination rate
...
Nmax=4.5 # upper bound for the population size in each time window (in log10 scale)
