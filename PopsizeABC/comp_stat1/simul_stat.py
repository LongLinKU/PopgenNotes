...
results=-np.ones(shape=[nb_rep,nb_dist+nb_size*nb_prob+n2/2+1],dtype='float') # missing statistic values will be set to 1. 
									      # this mostly concerns LD statistics, because it is sometimes impossible to find SNP pairs in a given distance bin.
print('Total numberb of statistics : '+str(nb_dist+nb_size*nb_prob+n2/2+1)+'\n')
...
for i in range(nb_rep):
    ...
    sys.stdout.flush()
    try:
        results[i,:]=popgen_abc.comp_stats_one_rep_macld(outfile_name_2_read,i,nb_seg2,L,n2,interval_list,size_list,prob_list,L,mac=mac,mac_ld=mac_ld)
    except:
