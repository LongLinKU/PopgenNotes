# This script computes the AFS, LD and IBS summary statistics described in the manuscript, from a sample of phased genomes with beagle output format.
# Although the phase information is actually not used here, we use this format because it was the one provided by the 1000 bull genomes project.
# The list of individuals used for the computation (a subset of the individuals included in the beagle files) must be provided in a separate file.
# The population of each individual in the beagle files must be provided in the first column of a file at ped format.
...
# For this example the beagle files are Chr29.map.gz and Chr29.phase.gz.
# The list of animals is list_indiv_Jersey.txt, the ped file is indiv.ped and the result file is example_vcf_Jersey_n30_mac6_macld6.stat.
...
pop='Jersey'
list_ani=hapIO.read_list('../cattle_data/list_indiv_Jersey.txt') # list of diploid animals used for computing the summary statistics
...
nb_snp=0
Lchr=0
for chro in range(29,30):
    # store data and pre-analyse it
    print 'Processing chromosome ',chro
    infile_map='../cattle_data/Chr'+str(chro)+'.map.gz'
    infile_phase='../cattle_data/Chr'+str(chro)+'.phase.gz'
    [mydata,mymap]=hapIO.parseBeagleFiles_allSNP('Chr'+str(chro),infile_map,infile_phase,includeInd=list_ani)
    hapIO.parsePedFile_nogeno('../cattle_data/indiv.ped',mydata)  
    u=hapdata.Haplos_and_counts(mydata,mymap,pop,mac=mac)
    pos_list.append(u[0][0])
    count_list.append(u[1][0])
    hap_list.append(u[2][0])
    p=len(u[0][0])
    nb_snp+=p
    Lchr+=u[0][0][p-1]
    u=hapdata.Haplos_and_counts(mydata,mymap,pop,mac=mac_ld)
    pos2_list.append(u[0][0])
    hap2_list.append(u[2][0])
    sys.stdout.flush()
...
sys.stdout.flush()
u=ss.break_chr(pos_list,hap_list,L)
pos_list=u[0]
hap_list=u[1]
u=ss.break_chr(pos2_list,hap2_list,L)
pos2_list=u[0]
hap2_list=u[1]
print 'Broke chromosomes in independent segments of size '+str(L)+', current time :'+time.ctime()
sys.stdout.flush()
geno_list=ss.hap_to_geno_sorted(hap_list)
geno2_list=ss.hap_to_geno_sorted(hap2_list)
print 'Genotypes cretaed, current time :'+time.ctime()
sys.stdout.flush()
...
# print the result
np.savetxt('res/example_beagle_'+pop+'_n'+str(n)+'_mac'+str(mac)+'_macld'+str(mac_ld)+'.stat',np.concatenate((np.array([np.float(nb_snp)/np.float(Lchr)],dtype='float'),res_afs[0],res_ld_zyg[0],res_ibs)),fmt='%.3e')
