...
######### from command line#####
pop = sys.argv[1]
inds_file = sys.argv[2]
import math
vcfs = sys.argv[3].split(';')
pedfile = sys.argv[4]
...
list_ani=IO.read_list(inds_file) # list of diploid animals used for computing the summary statistics
n=len(list_ani)*2 # haploid sample size
mac=int(math.ceil(n*0.1)) # minor allele count threshold for AFS and IBS statistics computation # n * 10%
mac_ld=int(math.ceil(n*0.2)) # minor allele count threshold for LD statistics computation # n * 20%
...
for infile_vcf in vcfs:
    # store data and pre-analyse it
    print 'Processing chromosome ',infile_vcf
...
# print the result
np.savetxt(outputDir+'/ex1_'+pop+'_n'+str(n)+'_mac'+str(mac)+'_macld'+str(mac_ld)+'.stat',np.concatenate((np.array([np.float(nb_snp)/np.float(Lchr)],dtype='float'),res_afs[0],res_ld_zyg[0],res_ibs)),fmt='%.3e')
